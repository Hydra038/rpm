      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-2">
            <input
              type="text"
              placeholder="Enter your 17-digit VIN number (e.g., 1HGCM82633A123456)"
              value={vin}
              onChange={(e) => {
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-gray-900">{vehicleInfo.make}</p>
                </div>
                <div>
                  <span className="font-medium text-gray-600">Model:</span>
                  <p className="text-gray-900">{vehicleInfo.model}</p>
                </div>
                <div>
                  <span className="font-medium text-gray-600">Year:</span>
                  <p className="text-gray-900">{vehicleInfo.year}</p>
                </div>
                <div>
                  <span className="font-medium text-gray-600">Engine:</span>
                  <p className="text-gray-900">{vehicleInfo.engine}</p>
                </div>
                <div>
                  <span className="font-medium text-gray-600">Transmission:</span>
                  <p className="text-gray-900">{vehicleInfo.transmission}</p>
                </div>
                <div>
                  <span className="font-medium text-gray-600">Fuel Type:</span>
                  <p className="text-gray-900">{vehicleInfo.fuelType}</p>
                </div>
              </div>
              ...
              {showSaveForm && (
                <div className="mt-4 pt-4 border-t border-blue-200">
                  <div className="flex gap-2 items-end">
                    ...
                    <Button size="sm" onClick={saveVehicle}>Save</Button>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              My Garage ({savedVehicles.length} vehicles)
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {savedVehicles.map((vehicle) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <h5 className="font-medium text-gray-900">{vehicle.nickname}</h5>
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={(e) => {
                  ...
                  <div className="text-sm text-gray-600 space-y-1">
                    <p>{vehicle.vehicle_info.year} {vehicle.vehicle_info.make} {vehicle.vehicle_info.model}</p>
