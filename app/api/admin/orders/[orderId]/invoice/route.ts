...
export async function GET(
  ...
  try {
    ...
    console.log('Generating admin invoice for order:', orderId);
    ...
    if (!itemsError && items && items.length > 0) {
      ...
      orderItems = [{
        ...
        description: '',
        parts: {
          name: order.product_name || 'Auto Parts',
          category: 'Auto Parts',
          description: ''
        }
      }];
    ...
    // Allow invoice generation for any payment status (not just paid)
    // This allows admins to generate invoices for pending/partial payments
    ...
    return new NextResponse(invoiceHTML, {
      headers: {
        ...
        'Content-Disposition': `inline; filename="admin-invoice-${orderId}.html"`
      }
    });
        ...
        <table class="items-table">
            ...
            <tbody>
                ${orderData.order_items.map((item: any) => `
                    <tr>
                        <td>
                            <strong>${item.product_name || item.parts?.name || 'Product'}</strong>
                            ${item.description || item.parts?.description ? `<br><small style="color: #666;">${item.description || item.parts?.description}</small>` : ''}
                        </td>
                        <td>${item.category || item.parts?.category || 'Auto Parts'}</td>
                        <td>${item.quantity}</td>
